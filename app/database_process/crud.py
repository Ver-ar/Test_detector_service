...
async def create_image(faces: int):
    async with engine.begin() as conn:
        result = (
            await conn.execute(image_table.insert(), {"faces": faces})
...
async def create_users(faces, user_id):
    exist = select(bot_table).where(
        bot_table.c.face_from_user == faces, bot_table.c.user_id == user_id
    )
    async with engine.begin() as conn:
        ...
        if result_ex == None:
            await conn.execute(
                bot_table.insert(), {"face_from_user": faces, "user_id": user_id}
            )
