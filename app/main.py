...
from database_process.crud import (
    count_image_faces,
    create_image,
    get_image_with_id,
    del_image,
    ...
    await asyncio.gather(
        *[
            app.bot.send_message(
                id, f"В базу добавлено фото с id: {item}, количество лиц: {faces}"
            )
            for id in users_id
        ]
    )
...
async def count_item_faces(
    faces: int = Path(..., title="The faces on the image to get")
) -> dict:
...
async def get_item(image_id: int = Path(..., gt=0)) -> dict:
    ...
    if images_db is None:
        raise HTTPException(
            status_code=404, detail="Image not found, id was be deleted"
        )
    else:
        return images_db
